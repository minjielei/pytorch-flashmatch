...
class Converter(object):
    def __init__(self, det_file='../data/detector_specs.yml', root_file = 'PhotonLibrary-20201209.root'):
        ...
        self.nvoxels = self.detector['PhotonLibraryNvoxels']
        self.nchannels = self.detector['PhotonLibraryNOpDetChannels']
        self._min = self.detector['PhotonLibraryVolumeMin']
        self._max = self.detector['PhotonLibraryVolumeMax']
    ...
    def convert(self, outfile):
        ...
        hf.create_dataset('max', data=np.array(self._max))
        hf.create_dataset('min', data=np.array(self._min))
        hf.create_dataset('numvox', data=np.array(self.nvoxels))
        hf.create_dataset('vis', data=self._vis)
